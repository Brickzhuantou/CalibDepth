...
def merge_inputs(queries):
    ...
    returns = {key: default_collate([d[key] for d in queries]) for key in queries[0]
               if key != 'pc_target' and key != 'rgb' and key != 'pc_source' and key != 'depth_gt' and key != 'depth_img'}
    for input in queries:
        pc_target.append(input['pc_target'])
        pc_source.append(input['pc_source'])
        imgs.append(input['rgb'])
        depth_gt.append(input['depth_gt'])
        depth_img.append(input['depth_img'])
        # if 'reflectance' in input:
            # reflectances.append(input['reflectance'])
    returns['pc_target'] = pc_target
    returns['pc_source'] = pc_source
    returns['rgb'] = imgs
    returns['depth_gt'] = depth_gt
    returns['depth_img'] = depth_img
